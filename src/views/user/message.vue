<template>
    <div v-loading="loading">
            ...
            <template #header>
                <div class="card-header">
                    <el-badge :value="item.is ? 'new' : ''">
                        <el-tag>{{ item.name }}</el-tag>{{ item.date }}
                    </el-badge>
                    <div>
                        <el-tooltip content="设置已读" placement="top-start">
                            <el-button text type="success" @click.once="handleChang(index)">已读</el-button>
                        </el-tooltip>
                        <el-tooltip content="点击回复" placement="top-start">
                            <el-button text type="primary" @click="handleReply(index)">回复</el-button>
                        </el-tooltip>
                        <DeleteButton @delete="handleDelete(index)" text type="danger">删除</DeleteButton>
                    </div>
                </div>
            </template>
            {{ item.content }}
        ...
        <el-dialog v-model="dialogVisible" width="30%">
            ...
            <el-input v-model="text" autosize type="textarea" placeholder="请输入回复内容" />
            <template #footer>
                <span class="dialog-footer">
                    <CancelButton @cancel="handleCancel" size="small"/>
                    <el-button type="primary" @click="handleSure()" size="small">发 送</el-button>
                </span>
            </template>
        </el-dialog>
    </div>
